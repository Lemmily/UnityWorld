...
public class TerrainObjectSpriteController : MonoBehaviour {

    //TODO: FIX "TerrainObject" - weird name - waht I actually mean is buildings/sites/places/etc
    ...
    public Dictionary<Dungeon, GameObject> dungeonToGameObject;
    private Dictionary<City, GameObject> cityToGameObject;
    ...
    void Start() {
        ...
        dungeonToGameObject = new Dictionary<Dungeon, GameObject>();
        cityToGameObject = new Dictionary<City, GameObject>();

        tile_container = new GameObject();
        tile_container.transform.SetParent(this.transform);
        tile_container.name = "OverworldObjects";
        pooling = true;
        setup = false;
        tileSize = 1;
    ...
    public void Update() {
        foreach (Dungeon dun in World.GetDungeons()) {
            if (dungeonToGameObject.ContainsKey(dun)) {
                GameObject go = dungeonToGameObject[dun];
                ...
                go.transform.localPosition = new Vector3(dun.x, dun.y, 0);
                sr.sortingLayerName = "TerrainObjects";
                dungeonToGameObject.Add(dun, go);
            }
        }
        foreach (City city in World.GetCities()) {
            if (cityToGameObject.ContainsKey(city)) {
                GameObject go = cityToGameObject[city];
                SpriteRenderer sr = go.GetComponent<SpriteRenderer>();
